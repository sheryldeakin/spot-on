...
| Pairing by | Right |
|---|---|
| appearance, with position breaking ties | 98.4% |
...
Appearance matching compares a 12x12 contrast-normalised thumbnail of each element, on absolute correlation so a card inverted for dark mode still matches itself. Left unbounded it goes the other way and pairs a label with its twin across the canvas: on six real runs it reached 1174px for a pair and made 45 pairings more than a quarter of a page apart, all of them junk. Distance is therefore priced into the cost and pairs above a total cost are refused; the two settings were swept together against both the synthetic pages and the real ones (`scripts/match_sweep.py`), and the chosen pair is the best of those that make no cross-page pairing at all on the real runs.
...
- **An element whose words changed can be lost.** Pairing by appearance is what makes a moved element recognisable, and it has the matching weakness: change the text inside a box and it may read as a different thing. That is the one scenario in the trial where it scores below position matching (94.4% against 100%), and it costs a "this moved" sentence, replaced by a "this is missing" one. It stays the better trade because rebuilds move things far more often than they reword them.
