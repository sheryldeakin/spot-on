...
- **Moving content sets a ceiling, and Spot On measures it.** The first time it scores a running page it screenshots that page twice and scores one against the other. A still page comes back at 99 or above. A real app with an animated hero came back at **77.3, with 3.3% of pixels moving between identical captures**, so nothing on that page could ever score above 77.3 and the report says so instead of reporting a 25-point gap that is not there. Freeze animations, carousels and live data, or point at a still section. Running pages are loaded with `--force-prefers-reduced-motion`, which helps only if the app honours it; `SPOT_ON_CHROME_FLAGS` passes anything else the browser needs.
