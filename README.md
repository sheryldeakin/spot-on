...
- Takes a line saying **what the rebuild may build with**, sent with every round: inline SVG for icons and for curved or radial shapes, `conic-gradient` for dials, blur and rgba for translucent panels. Left to guess, a model writes plain boxes, so a gauge comes out square and a wifi glyph comes out missing, and the report then reports the square box rather than the reason for it. The default keeps everything drawn by the page itself, because anything fetched makes a run depend on the network.
