...
- **Scores each ninth of the page separately**, because one number over a whole page is an average and an average hides where the work is. A rebuilt dashboard scored 70.7 while its own regions ran from 41 to 73, and the weakest part was the navigation bar rather than anything the headline suggested. It also explains why a big page seems to stall: a region worth a tenth of the pixels can be fixed completely and move the total by two or three points. Region scores compare with each other, not with the page total, which is measured over the whole page at once.
