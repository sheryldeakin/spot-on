...
**Or all of them at once.** When more than one is installed, the picker offers "all of them, one rewrite each": a round then spreads its draws across the different models instead of sampling one model several times, and each attempt records which one wrote it. Three draws from one model are three samples of the same habits; three models fail differently. The scorer still picks the winner, so this buys more varied drafts rather than better judgement. `SPOT_ON_PANEL` does the same from the command line.
