...
Set `SPOT_ON_PANEL` and a round spreads its draws across whichever of these the machine has rather than sampling one model several times, preferred agent first. Each attempt records which one wrote it. The scorer still picks the winner, so this buys more varied drafts, not better judgement.
